...
function CourseBuilderScreen({state, dispatch}) {
    ...
    return (
        <main className="container d-flex align-items-center course-builder flex-column">
            {state.createdCourse.image.length ?
                <img src={`http://localhost:3001/${state.createdCourse.image}`} className="img-fluid mb-3"/> : null}
            ...
            <div className="w-100 shadow p-3 mb-3 bg-white rounded">
                <div>
                    ...
                    <Input placeholder={"Nazwa kursu"} value={state.createdCourse.name}
                           onInput={(e) => coreCourseFieldDispatcher('name', e.target.value)} type="input"/>
                    <Input placeholder={"Opis kursu / wstęp"} value={state.createdCourse.description}
                           onInput={(e) => coreCourseFieldDispatcher('description', e)} type="tiny"/>
                </div>
            </div>
