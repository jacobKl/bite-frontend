...
function LoginScreen({ state, dispatch }) {
  return (
    <main className="w-100 h-100 d-flex align-items-center justify-content-center">
      <form onSubmit={(e) => submitForm(e, "login", dispatch, state.user.token)} className="d-flex flex-column paper text-center">
        <h3>Witaj!</h3>
        <input type="text" name="username" className="form-control mb-1" placeholder="Nazwa uzytkownika" />
